			...
			if ($rVat = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($dbVat)) {
				if ($rPriceShipping['product_type'] == 24)	{ // print or non-digital
					...
					if ($rShippingJournal = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($dbShippingJournal)) {
						$updateData['unit_price'] = $this->getEntryPrices($rShippingJournal['price'], $currencyInitArr[2], $rVat['vat'], $currencyInitArr[0]);
						if($rShippingJournal['shipping'] > 0){
							$updateData['shipping_cost'] = $this->getEntryPrices($rShippingJournal['shipping'], $currencyInitArr[2], $rVat['vat'], $currencyInitArr[0]);
						}
						else {
